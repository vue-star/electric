        ...
        methods: {
            ...
            surportHandle(item) {
                ...
                item.render = (h, params) => {
                    ...
                    const selectId = params.tableData[params.index][params.column.id] // 配电室信息判断数据是否存在，存在就显示删除按钮
                    return selectId === 0 ? '' : h('div', btns.map(item => item(h, params, this)))
                }
                return item
            },
            ...
            setDefaultSearchKey() {
                this.searchKey = this.columns[0].key !== 'handle' ? this.columns[0].key : (this.columns.length > 1 ?
                    this.columns[1].key : '')
            },
            handleClear(e) {
